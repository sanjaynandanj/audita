...
_TMP = tempfile.mkdtemp(prefix="audita-test-wq-")
os.environ.setdefault("AUDITA_DATA_DIR", _TMP)
os.environ.pop("GEMINI_API_KEY", None)

from fastapi.testclient import TestClient  # noqa: E402

from app.books.store import new_txn  # noqa: E402
from app.main import app, close_store, ledger_store  # noqa: E402
...
def _items(kind=""):
    body = client.get("/api/workqueue").json()
    if kind:
...
class TestWorkqueue:
    def test_invoice_draft_appears_then_clears_on_confirm(self):
        res = client.post(
            "/api/invoices",
            data={"period": "2035-01"},
        ...
        drafts = [i for i in _items("invoice_draft") if i["ref"] == invoice_id]
        assert len(drafts) == 1
        ...
        assert not [i for i in _items("invoice_draft") if i["ref"] == invoice_id]

    def test_pending_txns_appear_with_period_ref(self):
        txn = new_txn("01-01-2035", "WQ MYSTERY NARRATION", "W1", Decimal("-4321"), "wq:1")
        ledger_store.import_txns("2035-02", [txn])
        pending = [i for i in _items("txns_pending") if i["ref"] == "2035-02"]
        assert len(pending) == 1
        assert pending[0]["agent"] == "bookkeeping"
        assert pending[0]["count"] == 1
        assert pending[0]["amount"] == "-4321"

    def test_touched_close_workbook_lists_open_items(self):
        client.post("/api/close/2035-03/item",
                    json={"key": "bank-recon", "done": True, "actor": "Asha"})
        wq = [i for i in _items("close_open") if i["ref"] == "2035-03"]
        assert len(wq) == 1
        total_items = len(close_store.load_or_create("2035-03").items)
        assert wq[0]["count"] == total_items - 1

    def test_untouched_close_workbook_is_not_queued(self):
        close_store.load_or_create("2035-04")
        assert not [i for i in _items("close_open") if i["ref"] == "2035-04"]

    def test_review_flags_and_recon_exceptions_queued(self):
        ...
        ledger_store.import_txns("2035-05", [txn])
        client.post("/api/review/2035-05")
        flags = [i for i in _items("review_flags") if i["ref"] == "2035-05"]
        assert len(flags) == 1
        ...
        recon = [i for i in _items("recon_exceptions") if i["ref"] == report_id]
        assert len(recon) == 1
    ...
    def test_totals_and_sort_order(self):
        body = _items()
