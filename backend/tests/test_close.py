...
class TestCloseWorkbook:
    def test_create_default(self, db_conn, org):
        store = CloseStore(db_conn, org)
        wb = store.load_or_create("2026-04")
        assert len(wb.items) == len(DEFAULT_ITEMS)
        assert wb.done_count == 0

    def test_mark_done_and_reopen(self, db_conn, org):
        store = CloseStore(db_conn, org)
    ...
    def test_invalid_period_rejected(self, db_conn, org):
        store = CloseStore(db_conn, org)
        with pytest.raises(ValueError):
            store.load_or_create("April 2026")

    def test_unknown_key(self, db_conn, org):
        store = CloseStore(db_conn, org)
        store.load_or_create("2026-04")
        with pytest.raises(KeyError):
            store.set_item("2026-04", "nope", True, actor="x")

    def test_periods_listed(self, db_conn, org):
        store = CloseStore(db_conn, org)
